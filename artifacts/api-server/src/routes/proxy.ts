...
// Models whose extended-thinking API differs (require thinking.type.adaptive
// instead of thinking.type.enabled). Until we add adaptive support, do NOT
// expose -thinking / -thinking-visible variants for these.
const ANTHROPIC_NO_THINKING_VARIANTS = new Set<string>(["claude-opus-4-7"]);
...
const CLAUDE_MODELS = ANTHROPIC_BASE_MODELS.flatMap((id) =>
  ANTHROPIC_NO_THINKING_VARIANTS.has(id)
...
for (const base of ANTHROPIC_BASE_MODELS) {
  ...
  if (!ANTHROPIC_NO_THINKING_VARIANTS.has(base)) {
...
function requireApiKey(req: Request, res: Response, next: () => void) {
  ...
  if (!providedKey) {
    res.status(401).json({ error: { message: "Missing API key (use Authorization: Bearer <key>, x-api-key, x-goog-api-key, or ?key= query param)", type: "invalid_request_error" } });
    return;
  }
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      if (backend.kind === "friend") {
        ...
        // Upstream Anthropic caps thinking output at 32000 tokens for some models
        // (e.g. opus-4-1). Keep thinking ≤ 32k and ensure budget room.
        const THINKING_OUTPUT_CAP = 32000;
        const defaultMaxTokens = thinkingEnabled
...
// POST /v1/models/:model:generateContent        — non-streaming, native Gemini format
// POST /v1/models/:model:streamGenerateContent  — streaming, native Gemini SSE format
// Aliased at /v1beta/... for standard Gemini SDK client compatibility
// ---------------------------------------------------------------------------

function makeGeminiNativeHandler(streaming: boolean) {
  return async (req: Request, res: Response) => {
    const startTime = Date.now();
    // With regex routes, capture groups are in req.params[0], req.params[1], ...
    const rawModel = (req.params as unknown as string[])[0] ?? "";

    if (!rawModel) {
      res.status(400).json({ error: { message: "Model name is required in the URL path", type: "invalid_request_error" } });
      return;
    }

    // Strip thinking suffix for actual Gemini API call
    const thinkingSuffix = rawModel.endsWith("-thinking-visible") ? "thinking-visible"
      : rawModel.endsWith("-thinking") ? "thinking" : null;
    const actualModel = thinkingSuffix
      ? rawModel.slice(0, rawModel.length - thinkingSuffix.length - 1)
      : rawModel;

    // Validate this is a Gemini model
    const provider = MODEL_PROVIDER_MAP.get(rawModel) ?? MODEL_PROVIDER_MAP.get(actualModel);
    if (provider !== "gemini") {
      res.status(400).json({ error: { message: `Model '${rawModel}' is not a Gemini model. Use /v1/chat/completions for other providers.`, type: "invalid_request_error" } });
      return;
    }

    if (!isModelEnabled(rawModel) && !isModelEnabled(actualModel)) {
      res.status(400).json({ error: { message: `Model '${rawModel}' is disabled`, type: "invalid_request_error" } });
      return;
    }
    ...
    try {
      ...
      // Inject thinking config if model suffix requested it
      if (thinkingSuffix) {
        const genCfg = (body.generationConfig ?? {}) as Record<string, unknown>;
        genCfg.thinkingConfig = { thinkingBudget: (genCfg.maxOutputTokens as number | undefined) ? Math.min(genCfg.maxOutputTokens as number, 32768) : 16384 };
        body.generationConfig = genCfg;
      }

      const { promptTokens, completionTokens, ttftMs } = await handleGeminiNative({
        res, model: actualModel, body, streaming, startTime,
      });

      pushRequestLog({
        method: "POST", path: req.path, model: actualModel, backend: "local",
        status: 200, duration: Date.now() - startTime,
        stream: streaming, promptTokens, completionTokens,
        ...(ttftMs !== undefined ? {} : {}),
        level: "info",
      });
    } catch (err: unknown) {
      ...
      pushRequestLog({
        method: "POST", path: req.path, model: rawModel, backend: "local",
        status: 500, duration: Date.now() - startTime,
        stream: streaming, level: "error", error: errMsg,
      });
      if (!res.headersSent) {
        res.status(500).json({ error: { code: 500, message: errMsg, status: "INTERNAL" } });
      } else {
        res.end();
      }
    }
...
// /v1/models/:model:generateContent  +  alias /v1beta/models/:model:generateContent
router.post(/^\/v1\/models\/([^/:]+):generateContent$/, requireApiKey, makeGeminiNativeHandler(false));
router.post(/^\/v1beta\/models\/([^/:]+):generateContent$/, requireApiKey, makeGeminiNativeHandler(false));

// /v1/models/:model:streamGenerateContent  +  alias /v1beta/models/:model:streamGenerateContent
router.post(/^\/v1\/models\/([^/:]+):streamGenerateContent$/, requireApiKey, makeGeminiNativeHandler(true));
router.post(/^\/v1beta\/models\/([^/:]+):streamGenerateContent$/, requireApiKey, makeGeminiNativeHandler(true));
...
// ---------------------------------------------------------------------------
// Gemini native endpoint handler
// Accepts native Gemini request body (contents / generationConfig / systemInstruction)
// Returns native Gemini response format — no OpenAI translation
// ---------------------------------------------------------------------------

async function handleGeminiNative({
  res, model, body, streaming, startTime,
}: {
  res: Response;
  model: string;
  ...
  type GeminiContents = Parameters<typeof client.models.generateContent>[0]["contents"];
  const contents = (body.contents ?? []) as GeminiContents;
  const generationConfig = (body.generationConfig ?? {}) as Record<string, unknown>;
  const systemInstruction = body.systemInstruction as unknown | undefined;
  ...
  if (streaming) {
    ...
    const genStream = await client.models.generateContentStream({ model, contents, config });
    for await (const chunk of genStream) {
      if (ttftMs === undefined && chunk.text) ttftMs = Date.now() - startTime;
      if (chunk.usageMetadata) {
        promptTokens = chunk.usageMetadata.promptTokenCount ?? 0;
        completionTokens = chunk.usageMetadata.candidatesTokenCount ?? 0;
      }
...
async function handleClaude({
  ...
  // Adaptive thinking budget — must be strictly less than max_tokens.
  // Cap at 16000, but if user passed a small max_tokens, scale down.
  const THINKING_BUDGET = Math.max(1024, Math.min(16000, maxTokens - 1024));
